...
import           Mollie.API.Retry
import           Mollie.API.Idempotency
import           Mollie.API.Webhooks
import           Servant.API
...
{-|
  ...
  Automatically applies exponential backoff for retryable errors.
-}
runMollie :: ClientEnv -> ClientM a -> IO (Either PaymentError a)
runMollie env apiFunction = retryIO defaultRetryConfig $ do
    res <- runClientM apiFunction env
    return $ mapLeft (fromResponseError . handleError) res
    where
